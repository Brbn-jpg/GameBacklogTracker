...
const GameCard = React.memo(({ game, onRemove }) => {
  ...
  return (
    ...
    >
      <button 
        ...
        className="absolute top-1 right-1 p-1 text-slate-500 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity z-10"
      ...
      {game.headerImage ? (
        <img
          ...
          alt={game.gameTitle}
          className="w-16 h-20 object-cover rounded bg-slate-950 shrink-0"
        />
      ) : (
        <div className="w-16 h-20 bg-slate-900 rounded flex items-center justify-center text-xs text-slate-600 font-bold shrink-0 text-center px-1">
          NO COVER
        </div>
      )}
